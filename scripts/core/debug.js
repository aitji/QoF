import { CommandPermissionLevel, CustomCommandParamType, CustomCommandStatus, DisplaySlotId, EquipmentSlot, MemoryTier, system, world } from "@minecraft/server";
...
system.run(() => {
    ...
    if (DEBUG) {
        ...
        system.beforeEvents.watchdogTerminate.subscribe((d) => {
            if (host && host.clientSystemInfo.memoryTier === MemoryTier.High)
                d.cancel = true;
    ...
    reg.registerCommand({
        ...
        switch (types) {
            case 'block':
                ...
                if (!target?.block)
                    return { status: CustomCommandStatus.Failure, message: 'cannot find target block in player direction within 12 blocks' };
            ...
            default:
                ...
                if (!item)
                    return { status: CustomCommandStatus.Failure, message: 'cannot find item in mainhand' };
