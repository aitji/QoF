import { world, system, ItemStack, BlockPermutation, EquipmentSlot, ItemLockMode, InputPermissionCategory, GameMode, EntityComponentTypes } from "@minecraft/server";
...
const setJump = (player, n) => {
    if (!PLAYER_JUMP.NO_JUMP_HOLD_CHEST)
        return;
    if (DEBUG)
        world.sendMessage(`§7${player.name} jump=${n}`);
    try {
...
const buildCarryItem = (blockTypeId, player, container) => {
    ...
    if (!container) {
        ...
        if (invText.length === 0)
            invText.push(`§r§7(empty container)`);
        if (uniqueTotal > MAX_DISPLAY)
            invText.push(`§r§7and ${uniqueTotal - MAX_DISPLAY} more...`);
        it.setLore(invText);
    }
...
const safeMove = (fromInv, fromI, toInv, toI) => {
    try {
        fromInv.moveItem(fromI, toI, toInv);
    }
    catch { }
};
const block2inv = (block, heldInv) => {
    ...
    for (let i = ourStart; i < ourEnd; i++) {
        ...
        if (it)
            safeMove(container, i, heldCont, i - ourStart);
    }
...
const inv2block = (heldInv, block, neighbourInv) => {
    ...
    if (container.size !== DOUBLE_CHEST_SIZE) {
        for (let i = 0; i < container.size; i++)
            safeMove(heldCont, i, container, i);
        heldInv.remove();
        return;
    }
    const halfSize = DOUBLE_CHEST_SIZE / 2;
    system.runTimeout(() => {
        ...
        for (let i = 0; i < halfSize; i++)
            safeMove(heldCont, i, dest, i);
        const their = neighbourInv[0] ?? [];
        for (let i = 0; i < their.length; i++)
            if (their[i])
                dest.setItem(halfSize + i, their[i]);
        heldInv.remove();
    }, 1);
};
export const chest_playerInteractWithBlock = (data) => {
    ...
    if (!block || !getInv(block) || equ.getEquipment(EquipmentSlot.Mainhand) || !player.isSneaking || player.hasTag(CARRY_TAG))
        return;
    data.cancel = true;
    system.runTimeout(() => {
        try {
            ...
            if (blockTypeId.includes('lit_'))
                return;
            if (blockTypeId === "minecraft:jukebox")
                return;
            ...
            player.setDynamicProperty('qof:chest.storage', undefined);
            if (DEBUG)
                world.sendMessage(`§cContainer pickup error: ${e}`);
        }
    }, 1);
};
export const chest_playerPlaceBlock = (data) => {
    const { player, block } = data;
    if (!block || !block?.getComponent("minecraft:inventory") || !player.hasTag(CARRY_TAG))
        return;
    if ((JSON.parse(player.getDynamicProperty('qof:chest.storage') ?? 0).selectedSlotIndex) !== player.selectedSlotIndex)
        return;
    ...
    try {
        ...
        if (blockTypeId === CHEST_ID) {
            ...
            if (dirs)
                for (const dir of [dirs.left, dirs.right]) {
                    const n = blockInDir(block, dir);
                    const container = getInv(n)?.container;
                    if (!container)
                        continue;
                    if (isFaceChest(n, facing))
                        neighbourSnapshots.push(copyInv(container));
                }
        }
        ...
        if (blockTypeId === CHEST_ID) {
            ...
            if (stateKey && stateVal !== null && stateVal !== undefined)
                placed.setPermutation(BlockPermutation.resolve(placed.typeId, { [stateKey]: stateVal }));
        }
        ...
        try {
            holdingEntity.teleport(player.location, { dimension: player.dimension });
        }
        catch (_) { }
        ...
        if (cache.getPlayer(player, 'gameMode') === GameMode.Creative) {
            ...
            if (!PLAYER_JUMP.NO_JUMP_HOLD_CHEST || APPLY_IMPULSE.ENABLED) {
                ...
                if (PLAYER_JUMP.ALLOW_JUMP_IN_WATER && player.isInWater) {
                    allowJump = true;
                    if (APPLY_IMPULSE.ENABLED)
                        player.applyImpulse(APPLY_IMPULSE.VECTOR);
                }
                // lava
                ...
                if (PLAYER_JUMP.ALLOW_JUMP_IN_SCAFFOLDING) {
                    if (block?.typeId === 'minecraft:scaffolding')
                        allowJump = true;
                }
                if (PLAYER_JUMP.ALLOW_JUMP_IN_LADDER) {
                    if (block?.typeId === 'minecraft:ladder')
                        allowJump = true;
                }
            }
            if (allowJump !== canJump)
                jumpSet(allowJump);
        }
...
export const chest_entityDie = (data) => {
    ...
    const inv = findInv(player);
    if (!inv)
        return player.removeTag(CARRY_TAG);
    ...
    system.runTimeout(() => {
        ...
        for (const en of entities) {
            ...
            if (!item || !item?.nameTag)
                continue;
            if (item?.nameTag === CONTAINER_NAMETAG)
                try {
                    ...
                    if (DEBUG)
                        world.sendMessage(`§c[chest] unexpected error while spawn normal container`);
                }
        }
    }, 1);
    player.removeTag(CARRY_TAG);
