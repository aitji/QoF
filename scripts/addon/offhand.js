import { BlockPermutation, Difficulty, Direction, EntityComponentTypes, EquipmentSlot, GameMode, ItemComponentTypes, PlatformType, system, world } from "@minecraft/server";
...
system.run(() => { for (const player of world.getAllPlayers())
    _initPlayer(player); });
function _initPlayer(player) {
    const { id } = player;
    if (sneakState.has(id))
        return;
    sneakState.set(id, { lastSneakTick: -999, wasSneaking: false });
}
const sneakWindow = {
    [PlatformType.Mobile]: DOUBLE_SNEAK_WINDOW_MOBILE,
    [PlatformType.Console]: DOUBLE_SNEAK_WINDOW_CONSOLE,
    [PlatformType.Desktop]: DOUBLE_SNEAK_WINDOW_DEFAULT
};
export function offhand_player(player, now) {
    ...
    const platform = plCache.platformType;
...
export const offhand_playerInteractWithEntity = (event) => {
    ...
    const equippable = player.getComponent(EntityComponentTypes.Equippable);
    const offhand = equippable.getEquipment(EquipmentSlot.Offhand);
    if ((target.typeId === "minecraft:cow" || target.typeId === "minecraft:mooshroom") &&
...
function canPlaceTorchOn(block) {
    ...
    if (block.permutation.matches(LIGHT))
        return true;
    const replace = ALLOW_REPLACE[block.typeId];
    if (replace === true)
        return true;
    if (replace === false)
        return block.below()?.isSolid ?? false;
    return false;
}
const delay = {};
export const offhand_playerInteractWithBlock = (data) => {
    ...
    if (!isFirstEvent) {
        ...
        if (playerDelay > system.currentTick)
            return;
    }
    ...
    if (itemStack) {
        ...
        if (typeId) {
            if (CAN_ALWAYS_USE.has(typeId))
                return;
            if (typeId.endsWith('_boat'))
                return;
            if (typeId.endsWith('minecart'))
                return;
            if (typeId.endsWith('harness'))
                return;
            if (typeId.endsWith('bundle'))
                return;
            ...
            if (typeId.endsWith('_chestplate'))
                return;
            if (typeId.endsWith('_helmet'))
                return;
            if (block && block.typeId === 'minecraft:jukebox' && typeId.startsWith('minecraft:music_disc_'))
                return;
        }
        // ignore foods
        if (food) {
        ...
        // shovel/hoe
        if (block && block?.hasTag('dirt') &&
            itemStack && (itemStack.hasTag('minecraft:is_shovel') ||
            itemStack.hasTag('minecraft:is_hoe') && !(block.typeId === 'minecraft:farmland' ||
                block.typeId === 'minecraft:soul_sand' ||
                COCOA_VALID_LOGS.has(block.typeId))))
            return;
        if (ITEMBUTBLOCK[typeId] === true)
            return;
        try {
            return !(itemStack && BlockPermutation.resolve(itemStack?.typeId) !== undefined);
        }
        catch { }
    }
    // handle offhand
...
const blockHandle = (data) => {
    ...
    system.run(() => {
        ...
        if (cache.getPlayer(player, 'gameMode') !== GameMode.Creative)
            player.runCommand(`replaceitem entity @s slot.weapon.offhand 0 ${typeId} ${offhandItem.amount - 1}`);
    });
};
const seedsHandle = (data) => {
    ...
    if (!plantEntry && !isCocoa)
        return;
    if (isCocoa) {
        if (!COCOA_VALID_LOGS.has(block?.typeId))
            return;
        if (block && itemStack && itemStack.hasTag('minecraft:is_axe') && (block.typeId === 'minecraft:jungle_log' ||
            block.typeId === 'minecraft:jungle_wood'))
            return;
        const target = block[BLOCKFACE_TO_DIR[blockFace]](1);
        if (!target || !(target.isAir || target.permutation.matches(LIGHT_BLOCK)))
            return;
        data.cancel = true;
        system.run(() => {
            ...
            if (current.amount <= 1)
                ...
                player.runCommand(`replaceitem entity @s slot.weapon.offhand 0 ${typeId} ${current.amount - 1}`);
        });
        return;
    }
    ...
    if (!above || !(above.isAir || above.permutation.matches(LIGHT_BLOCK)))
        return;
...
const fireHandle = (data) => {
    ...
    if (cache && (cache.permutation.matches('minecraft:fire') || cache.permutation.matches('minecraft:soul_fire')) &&
...
const torchHandle = (data, creative) => {
    ...
    const reduceItem = () => {
        try {
            ...
            if (creative)
                return;
            if (offhandItem && offhandItem.amount <= 1)
                equ.setEquipment(EquipmentSlot.Offhand, undefined);
            else
                player.runCommand(`replaceitem entity @s slot.weapon.offhand 0 ${offhandItem.typeId} ${offhandItem.amount - 1}`);
        }
            ...
            if (DEBUG)
                console.warn('[OFFHAND] unknown case:', e);
        }
    };
    ...
    if (!blockId)
        return;
    if (blockId === true)
        blockId = offhandItem.typeId;
    if (block && cache.typeId !== block.typeId)
        return;
    if (block.isLiquid)
        return;
    const isLightBlock = block.permutation.matches(LIGHT);
    const isSolidOrLight = (block.isSolid || isLightBlock) && !isLightBlock;
    if (isSolidOrLight && NEED_SNEAK[block.typeId] && !player.isSneaking)
        return;
    if (isLightBlock) {
        data.cancel = true;
        return system.run(() => {
    ...
    if (isSolidOrLight) {
        ...
        system.run(() => {
            ...
            if (!target || !canPlaceTorchOn(target))
                return;
            reduceItem();
            target.setPermutation(BlockPermutation
                .resolve(blockId)
    ...
    if (replace === undefined)
        return;
    system.run(() => {
        if (replace === false && !(block.below()?.isSolid))
            return;
        if (canPlaceTorchOn(block))
            block.setPermutation(BlockPermutation.resolve(blockId).withState('torch_facing_direction', 'top'));
        reduceItem();
    });
};
