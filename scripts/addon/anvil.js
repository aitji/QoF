...
function getCache(typeId, direction) {
    ...
    if (perm === undefined) {
        try {
            perm = BlockPermutation.resolve(typeId).withState('minecraft:cardinal_direction', direction);
        }
        catch (e) {
            if (DEBUG)
                console.warn('[ANVIL] failed to resolve permutation for', typeId, e);
            perm = null;
        }
...
export const anvil_playerInteractWithBlock = (data) => {
    ...
    if (block &&
        ...
        if (!isFirstEvent) {
            ...
            if (REPAIRABLE_ANVIL[block.typeId] || itemStack && itemStack.typeId === ITEM_TYPEID)
                data.cancel = true;
        ...
        system.run(() => {
            ...
            try {
                ...
                if (DEBUG)
                    console.warn('[ANVIL] unknown case:', e);
            }
        });
    }
};
