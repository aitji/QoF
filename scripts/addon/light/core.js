...
import { checkRandom, clamp, roundLoc, RUNTIME, sumLoc } from "../../lib";
const { DEBUG, BLOCKFACE_TO_DIR, LIGHT: { LIGHT_WIKI: light, ENABLED, LIGHT_ENTITY, DECAY_LIGHT_TICK, REDUCE_LIGHT, LIGHT_RENDER_RADIUS, LIGHT_RENDER_PER_PLAYER, LIGHT_FIRE_LEVEL, LIGHT_REDUCE_LINEAR, FAIL_PARTICLE, PARTICLE_OFFSET, LIGHT_BLOCK, SOUND_FAIL, FAIL_SOUND_INTERVAL, LIGHT_PENDING_BATCH, LIGHT_PLAYER_BATCH } } = RUNTIME;
...
export const lightMap = new Map();
export const frameSet = new Set();
export const entityLights = new Map();
export const suppressedLocs = new Map();
...
export const suppressLight = (block, checkLightBlock = true, cleanLight = true, needTick = false, tick = system.currentTick) => {
    if (!ENABLED)
        return false;
    if (checkLightBlock && !block.permutation.matches(LIGHT_BLOCK))
        return false;
    ...
    if (!cleanLight)
        return true;
    if (needTick)
        system.run(() => block.setPermutation(AIR));
    else
...
function put_light(block, level, owner, force = false) {
    try {
        ...
        if (exp !== undefined && system.currentTick < exp)
            return;
        if (block.isLiquid && block.permutation.getState('liquid_depth') !== 0)
            return;
        if (block.permutation.matches('minecraft:lava') || block.permutation.matches('minecraft:fire'))
            return;
        if (lightMap.has(k)) {
        ...
        if (ownerId !== 'Infinity') {
            if (!entityLights.has(ownerId))
                entityLights.set(ownerId, new Set());
            entityLights.get(ownerId).add(k);
        }
        if (isLightable(block, isWater))
            block.setPermutation(lightPerm(level));
    }
    catch (_) { }
}
function spreadLight(block, level, en, height = 2, force = false) {
    ...
    const tryPut = (blo) => {
        ...
        if (seen.has(k))
            return;
        try {
            if (blo.below(1).typeId === 'minecraft:grass_path')
                return;
        }
        catch { }
...
export const light_pending = (tick) => {
    ...
    for (let i = 0; i < budget; i++) {
        ...
        try {
            ...
            if (v.time < 0) {
                ...
                if (lig <= 0) {
                    if (isLightable(block, v.isWater))
                        block.setPermutation(v.isWater ? WATER : AIR);
                    dead.push(k);
                    continue;
                }
                if (isLightable(block, v.isWater))
                    block.setPermutation(lightPerm(lig - LIGHT_REDUCE_LINEAR));
                v.time -= LIGHT_REDUCE_LINEAR;
                continue;
            }
            if (isLightable(block, v.isWater))
                block.setPermutation(lightPerm(v.level));
            v.time -= LIGHT_REDUCE_LINEAR;
        }
        catch { }
    }
    ...
    for (let i = 0; i < dead.length; i++) {
        ...
        const v = lightMap.get(k);
        const owner = v?.owner;
        if (owner && owner !== 'Infinity')
            entityLights.get(owner)?.delete(k);
        frozenKeys.delete(k);
        lightMap.delete(k);
...
function _buildEntityQueue(players, tick) {
    ...
    for (const pl of players) {
        ...
        for (const en of nearby) {
            if (!en || _excludedTypes.has(en.typeId))
                continue;
            if (seen.has(en.id))
                continue;
            seen.add(en.id);
...
export const light_player = (pl, tick) => {
    ...
    for (let i = 0; i < budget; i++) {
        ...
        if (en.getComponent(EntityComponentTypes.OnFire))
            lightLevel += LIGHT_FIRE_LEVEL * REDUCE_LIGHT;
        const glowEntity = LIGHT_ENTITY[en.typeId];
        if (glowEntity)
            lightLevel = Math.hypot(lightLevel, glowEntity.light * REDUCE_LIGHT);
        if (lightLevel <= 0)
            continue;
...
export const light_playerPlaceBlock = ({ block }) => {
    ...
    if (DEBUG)
        world.sendMessage(`§8add frame:§7 ${k}`);
};
export const light_processFrames = (_tick) => {
    ...
    for (let i = 0; i < dead.length; i++) {
        ...
        if (DEBUG)
            world.sendMessage(`§8clear frame: §7${k}`);
    }
};
