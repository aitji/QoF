import { EntityComponentTypes, world, ItemStack, system } from "@minecraft/server";
import { checkRandom, RUNTIME } from "../../lib";
const { DEBUG, SLICE_PREFIX, WATER_CONCRETE: { TYPEID_ENDSWITH, ITEM_PREFIX, KEEP_VELOCITY, BATCH_SIZE, PROCESS_DELAY, DONE_PARTICLE, DONE_SOUND, SLOW_BASE, SLOW_MULTIPLIER } } = RUNTIME;
...
export const powder_entitySpawn = ({ entity }) => {
    ...
    if (!typeId.endsWith(TYPEID_ENDSWITH) || !typeId.startsWith(ITEM_PREFIX))
        return;
    const color = typeId.slice(SLICE_PREFIX).split(TYPEID_ENDSWITH)[0];
    if (!color)
        return;
    if (DEBUG)
        world.sendMessage(`§7spawn: ${color} x${itemStack.amount}`);
    queue.set(entity.id, {
        color,
...
export const powder_pending = () => {
    ...
    for (let i = start; i < end; i++) {
        ...
        if (entry.readyAt === null) {
            ...
            if (DEBUG)
                world.sendMessage(`| wet delay ${(wetDelay(entry.amount) / 20).toFixed(2)}s.`);
        ...
        system.runTimeout(() => {
            if (!entity.isValid)
                return;
            const concrete = dimension.spawnItem(new ItemStack(`${ITEM_PREFIX}${color}_concrete`, amount), location);
            ...
            if (KEEP_VELOCITY)
                concrete.applyImpulse({ x: velocity.x, y: velocity.y, z: velocity.z });
            try {
                entity.remove();
            }
            catch {
