import { BlockComponentTypes, BlockPermutation, EquipmentSlot, GameMode, ItemComponentTypes, ItemStack, system, world } from "@minecraft/server";
...
const maybeFinish = (block, dim, loc) => {
    // ---
...
export const composter_playerInteractWithBlock = (data) => {
    ...
    if (player.isSneaking || !itemStack || block.typeId !== BLOCK_TYPEID)
        return;
    ...
    system.run(() => {
        ...
        if (success) {
            ...
            if (state === 6)
                maybeFinish(block, player.dimension, loc);
        }
        else
            playSound(player.dimension, SOUND_FILL, loc);
        ...
        if (!currItem || currItem.typeId !== itemStack.typeId) {
            ...
            if ((!removed || removed === 0) && success)
                setLevel(block, state - 1);
            else if (itemID.endsWith('_stew') || itemID.endsWith('_soup'))
        ...
        try {
            ...
            if (itemStack.amount <= 1) {
                if (itemID.endsWith('_stew') || itemID.endsWith('_soup'))
                    equ.setEquipment(EquipmentSlot.Mainhand, new ItemStack('bowl', 1));
                else
            ...
            if (DEBUG)
                console.warn('[composter] unknown case:', e);
        }
    });
};
export const composter_playerPlaceBlock = (data) => {
    const { block, dimension } = data;
    if (!block || block.typeId !== BLOCK_TYPEID)
        return;
    const value = `${dimension.id.substring(SLICE_PREFIX)}:${block.x}:${block.y}:${block.z}`;
    composterSet.add(value);
    world.setDynamicProperty(DATA_COMPOSTER_LOCATION, JSON.stringify([...composterSet]));
...
export const composter_pistonActivate = (data) => {
    ...
    system.run(() => {
        ...
        for (const { oldValue, newValue } of moves) {
            if (!composterSet.delete(oldValue))
                continue;
...
export const composter_pending = (tick) => {
    ...
    for (const v of [...composterSet]) {
        ...
        if (!hopper || !hopper.isValid || hopper.isAir || hopper.typeId !== HOPPER_TYPEID)
            continue;
        if (hopper.getRedstonePower() > 0)
            continue;
        const perm = hopper.permutation;
        if (perm.getState("powered_bit") || perm.getState("facing_direction") !== 0)
            continue;
        ...
        for (let i = 0; i < container.size; i++) {
            ...
            if (com?.compostingChance)
                return applyEarly(); // i'm bail
            ...
            if (bowlSlot === -1 && item.typeId === 'minecraft:bowl' && item.amount < 64)
                bowlSlot = i;
        }
        if (firstChanceItem === -1)
            return applyEarly();
        ...
        if (item.amount > 1)
            container.setItem(firstChanceItem, new ItemStack(itemID, item.amount - 1));
        else {
            ...
            if (isStew) {
                ...
                if (targetBowlSlot !== -1) {
                    ...
                    container.setItem(targetBowlSlot, new ItemStack('minecraft:bowl', existing ? existing.amount + 1 : 1));
                }
            }
        }
        ...
        if (success) {
            ...
            setLevel(block, state + 1);
            if (state === 6)
                maybeFinish(block, dimension, loc);
        }
        else
            playSound(dimension, SOUND_FILL, loc);
    }
    applyEarly();
};
//# sourceMappingURL=composter.js.map