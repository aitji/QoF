import { world, system, EquipmentSlot, ItemStack, BlockPermutation, EntityComponentTypes, GameMode } from "@minecraft/server";
...
const consumeOffhand = (player, seed, equippable) => {
    ...
    if (!offhandItem || offhandItem.typeId !== seed)
        return false;
    const newAmount = offhandItem.amount - 1;
    if (newAmount <= 0)
        equippable.setEquipment(EquipmentSlot.Offhand, undefined);
    else
        player.runCommand(`replaceitem entity @s slot.weapon.offhand 0 ${seed} ${newAmount}`);
    return true;
};
export const harvest_playerBreakBlock = (data) => {
    ...
    if (isHoe || isAxe) {
        if (isAxe && block?.typeId !== 'minecraft:cocoa')
            return;
            ...
            if (growth && growth >= level) {
                system.run(() => {
                    ...
                    const apply = () => {
                        ...
                        if (plantTypeId === 'minecraft:cocoa')
                            target.setPermutation(resolveCocoaPermutation(target, originalDir));
                        else
                            target.setPermutation(BlockPermutation.resolve(plantTypeId));
                    };
                    if (!LOSS_SEED)
                        return apply();
                    if (consumeOffhand(player, plantSeed, equippable))
                        return apply();
                    ...
                    if (slot !== undefined) {
                        ...
                        for (const en of itemEntity) {
                            ...
                            if (!enItem || !en?.isValid)
                                continue;
                            if (enItem?.typeId === plantSeed) {
                                const newItem = reduceItem(enItem);
                                if (!newItem || newItem.typeId === 'minecraft:air')
                                    continue;
                                try {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        if (DEBUG)
                                            world.sendMessage(`[harvest] cannot remove ${plantSeed} ${_}`);
                                    }
                                }
                                finally {
                                    return apply();
